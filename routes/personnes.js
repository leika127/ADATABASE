...
router.get("/:id", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM personnes WHERE id = $1", [
      req.params.id,
    ]);
...
router.post("/", async (req, res) => {
  ...
  if (isNaN(Number(age)) || Number(age) < 18) {
    return res
      .status(400)
      .json({
        error: "L'âge doit être un nombre valide supérieur ou égal à 18",
      });
  }
...
router.delete("/:id", async (req, res) => {
  try {
    const result = await pool.query(
      "DELETE FROM personnes WHERE id = $1 RETURNING *",
      [req.params.id],
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ erreur: "Personne introuvable" });
